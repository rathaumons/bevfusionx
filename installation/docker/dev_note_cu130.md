...
> - Use [`Dockerfile.cu130`](Dockerfile.cu130) + [`docker-compose-cu130.yml`](docker-compose-cu130.yml) for reproducible builds (see [`README.md`](README.md))
...
- Create and run `bev-train-cu130` container with a mounted workspace `home/$USER/docker/bev_train_cu130:/workspace`:
...
- Install all necessary packages and miniconda inside the running container `bev-train-cu130`:
...
- Enter the container `bev-train-cu130` from host:
...
- Install custom [`cumm`](https://github.com/FindDefinition/cumm.git) and [`spconv`](https://github.com/traveller59/spconv.git) with CUDA 13.0:
...
- Export to `bev_train_cu130_2026.tar` from host:

  ```bash
  docker commit bev-train-cu130 bev-train:cu130
  docker save -o bev_train_cu130_2026.tar bev-train:cu130
...
- Import from `bev_train_cu130_2026.tar` in host:

  ```bash
  sha256sum -c bev_train_cu130_2026.tar.sha256
  docker load -i bev_train_cu130_2026.tar
  ```

- Start the container with a mounted workspace `home/$USER/docker/bev_train_cu130:/workspace`:
