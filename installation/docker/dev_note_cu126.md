...
- Pull the [official NVIDIA CUDA 12.6.3](https://hub.docker.com/layers/nvidia/cuda/12.6.3-cudnn-devel-ubuntu20.04) on host:
...
- Install all necessary packages and miniconda inside the running container `bevfusionx-cu126`:
...
- Enter the container `bevfusionx-cu126` from host:

  ```bash
  docker restart bevfusionx-cu126
  docker exec -it bevfusionx-cu126 bash
  ```
...
- Install [PyTorch](https://pytorch.org/) 2.11.0 + CUDA 12.6 (Max support: `compute_90`, `sm_90`):
...
- Install [MMCV v1.7.4](https://github.com/rathaROG/mmcv/releases/tag/v1.7.4-bevfusionx) with CUDA:
  ...
  ```bash
  pip install --extra-index-url https://rathaumons.github.io/bevfusionx-index/cu126/ mmcv-full==1.7.4
  ```
...
- Install custom [`cumm`](https://github.com/rathaROG/cumm-gpu) and [`spconv`](https://github.com/rathaROG/spconv-gpu) with CUDA 12.6:

  ```bash
  pip install --extra-index-url https://ratharog.github.io/cumm-spconv/ cumm-cu126==0.9.1
  pip install --extra-index-url https://ratharog.github.io/cumm-spconv/ spconv-cu126==2.4.1
  ```

- Install `flash-attn==1.2.1` and `setuptools==59.5.0`:

  ```bash
  pip install --extra-index-url https://rathaumons.github.io/bevfusionx-index/cu126/ flash-attn==1.2.1
...
- Export to `bevfusionx_cu126.tar` from host:

  ```bash
  docker commit bevfusionx-cu126 bevfusionx:cu126
  docker save -o bevfusionx_cu126.tar bevfusionx:cu126
  sha256sum bevfusionx_cu126.tar > bevfusionx_cu126.tar.sha256
  ```
...
- Import from `bevfusionx_cu126.tar` in host:

  ```bash
  sha256sum -c bevfusionx_cu126.tar.sha256
  docker load -i bevfusionx_cu126.tar
  ```

- Start the container with a mounted workspace `home/$USER/docker/bevfusionx-cu126:/workspace`:
