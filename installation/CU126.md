...
- Only Python 3.11 -> `conda create --name bevfusionx python=3.11`
...
- Build `bevfusionx`:
  ...
  - Full control: Use the `BEVX_CUDA_ARCH_LIST` environment variable to specify custom architectures (e.g., "8.9;12.0"; see [`nvidia-arch`](https://github.com/rathaROG/nvidia-arch) docs for syntax). For example, for normal consumer GPUs such as **NVIDIA RTX 3000/4000** series and workstation GPUs such as **NVIDIA RTX A6000** and **NVIDIA RTX 6000 Ada**:

    ```bash
    BEVX_CUDA_ARCH_LIST="8.6;8.9+PTX" python setup.py develop
    ```

  - Filtered auto-detection: Use `BEVX_GPU_TYPE` to specify the GPU type (`gpu_type`) and `BEVX_MIN_SM` to set the minimum SM number (`min_sm`) for auto-filtering via the [`nvidia-arch`](https://github.com/rathaROG/nvidia-arch) package. For example, for Data Center GPUs from **NVIDIA T4, A100, H100, ...** to **NVIDIA B300/GB300**:
